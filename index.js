...
function getArtistByIndex(array1, num) {
  var num;
  console.log("the artist at index " + num + " is " + array1[num].name);
  return "the artist at index " + num + " is " + array1[num].name;
}  
...
//Was unable to figure this one out. wasn't sure how turn the string into an int since it had - between them.
function get20s(array2){
  const namearray = [];
  for (let a = 0; a < array2.length; a++){
    //console.log(array5[a]);
    //var num1 = parseint(array2[a].years);
    if (array2[a].years >= 1900 && array2[a].years >= 2000){
        namearray.push(array2[a].name);
      }
    else{
      namearray.push("nope not it");
    }
  }
...
function removeArtist(array3, num){
...
function addArtist(array4){
  array4.push({ 
      id: 20,
      name: "Constance Nwaigwe", 
      years: "2001 - 2021",
      genre: "Web Design", 
...
function lotsOfArt(array5){
  const namearr = [];
  for (let a = 0; a < array5.length; a++){
    //console.log(array5[a]);
    if (array5[a].paintings > 100){
        namearr.push(array5[a].name);
      }
  }
  console.log(namearr);
  return namearr;
